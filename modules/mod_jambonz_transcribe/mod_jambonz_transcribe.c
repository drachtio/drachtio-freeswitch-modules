/* 
 *
 * mod_jambonz_transcribe.c -- Freeswitch module for using jb streaming transcribe api
 *
 */
#include "mod_jambonz_transcribe.h"
#include "jb_transcribe_glue.h"

/* Prototypes */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_jambonz_transcribe_shutdown);
SWITCH_MODULE_LOAD_FUNCTION(mod_jambonz_transcribe_load);

SWITCH_MODULE_DEFINITION(mod_jambonz_transcribe, mod_jambonz_transcribe_load, mod_jambonz_transcribe_shutdown, NULL);
...
static void responseHandler(switch_core_session_t* session, 
	...
	switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "transcription-vendor", "jambonz");
...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
	...
	case SWITCH_ABC_TYPE_CLOSE:
		{
			...
			jb_transcribe_session_stop(session, 1,  tech_pvt->bugname);
	...
	case SWITCH_ABC_TYPE_READ:
		...
		return jb_transcribe_frame(session, bug);
		break;
...
SWITCH_STANDARD_API(jb_transcribe_function)
{
...
SWITCH_MODULE_LOAD_FUNCTION(mod_jambonz_transcribe_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "jambonz Speech Transcription API loading..\n");

  if (SWITCH_STATUS_FALSE == jb_transcribe_init()) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "Failed initializing jb speech interface\n");
	}

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "jambonz Speech Transcription API successfully loaded\n");

	SWITCH_ADD_API(api_interface, "uuid_jambonz_transcribe", "jambonz Speech Transcription API", jb_transcribe_function, TRANSCRIBE_API_SYNTAX);
	switch_console_set_complete("add uuid_jambonz_transcribe start lang-code [interim|final] [stereo|mono]");
	switch_console_set_complete("add uuid_jambonz_transcribe stop ");
...
/*
  ...
  Macro expands to: switch_status_t mod_jambonz_transcribe_shutdown() */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_jambonz_transcribe_shutdown)
{
	jb_transcribe_cleanup();
