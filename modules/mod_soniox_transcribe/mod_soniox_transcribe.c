/* 
 *
 * mod_soniox_transcribe.c -- Freeswitch module for real-time transcription using soniox's gRPC interface
 *
 */
#include "mod_soniox_transcribe.h"
#include "soniox_glue.h"
#include <stdlib.h>
#include <switch.h>
...
SWITCH_MODULE_DEFINITION(mod_soniox_transcribe, mod_transcribe_load, mod_transcribe_shutdown, NULL);
...
static void responseHandler(switch_core_session_t* session, const char * json, const char* bugname, 
	...
	if (0 == strcmp("vad_detected", json)) {
		...
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "transcription-vendor", "soniox");
	}
...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
	...
	case SWITCH_ABC_TYPE_CLOSE:
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Got SWITCH_ABC_TYPE_CLOSE, calling soniox_speech_session_cleanup.\n");
			soniox_speech_session_cleanup(session, 1, bug);
	...
	case SWITCH_ABC_TYPE_READ:
		...
		return soniox_speech_frame(bug, user_data);
		break;
...
static switch_status_t do_stop(switch_core_session_t *session, char *bugname)
{
	...
	if (bug) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Received user command command, calling soniox_speech_session_cleanup (possibly to stop prev transcribe)\n");
		status = soniox_speech_session_cleanup(session, 0, bug);
  ...
  char* lang, int interim, char* bugname)
{
	...
	if (SWITCH_STATUS_FALSE == soniox_speech_session_init(session, responseHandler, samples_per_second, flags & SMBF_STEREO ? 2 : 1, lang, interim, bugname, &pUserData)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Error initializing soniox speech session.\n");
		return SWITCH_STATUS_FALSE;
	}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_transcribe_load)
{
  ...
  if (SWITCH_STATUS_FALSE == soniox_speech_init()) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "Failed initializing soniox speech interface\n");
	}

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Soniox Speech Transcription API successfully loaded\n");

	SWITCH_ADD_API(api_interface, "uuid_soniox_transcribe", "Soniox Speech Transcription API", transcribe_function, TRANSCRIBE_API_SYNTAX);
	switch_console_set_complete("add uuid_soniox_transcribe start lang-code");
	switch_console_set_complete("add uuid_soniox_transcribe stop ");
...
/*
  ...
  Macro expands to: switch_status_t mod_soniox_transcribe_shutdown() */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_transcribe_shutdown)
{
	soniox_speech_cleanup();
